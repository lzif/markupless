...
const Hub = () => {
	return div(
		...
		h1("Markupless Framework Showcase", {
			style: {
				textAlign: "center",
				color: "#2c3e50",
			},
		}),
		p("Select a demo below to see Markupless in action:", {
			style: {
				textAlign: "center",
				marginBottom: "30px",
			},
		...
		section(
			...
			h1("Features Active"),
			ul(
				li("Plugin System: LoggerPlugin is active (check console)"),
				li("Router: Managing this navigation"),
				li("State: Managing reactive UI updates"),
			),
		),
	);
};
...
const DemoCard = (title: string, desc: string, path: string) => {
	...
	return div(
		...
		h1(title, { style: { fontSize: "1.2rem", margin: "0 0 10px 0" } }),
		p(desc, { style: { color: "#666", fontSize: "0.9rem" } }),
		button("Launch", {
			style: {
				marginTop: "15px",
				padding: "8px 16px",
