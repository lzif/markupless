...
describe("Integration Test: Full App Flow", () => {
	...
	it("should render a component, handle state changes, and update DOM", () => {
		...
		const CounterComponent = () => {
			return new BaseElement("div").attr("id", "counter-container").with(
	...
	it("should handle routing between pages", async () => {
		const PageOne = () =>
			new BaseElement("div").text("Page One").attr("id", "page-1");
		const PageTwo = () =>
			new BaseElement("div").text("Page Two").attr("id", "page-2");
